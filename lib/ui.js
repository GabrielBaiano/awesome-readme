...
/**
 * Wizard Mode - Walks through steps defined in config
 */
export async function runWizardMode(rl) {
  ...
  for (const step of config.menus.wizard) {
    if (step.type === 'select-language') {
      console.log(colorize(`\n${step.question}`, 'yellow'));
      const availableLangs = Object.keys(config.languages);
      availableLangs.forEach((l, i) => console.log(`${i + 1}. ${config.languages[l].name} (${l})`));
      
      const choice = await askQuestion(rl, `Choice (1-${availableLangs.length}): `);
      mainLang = availableLangs[parseInt(choice) - 1] || 'en';
    } 
    else if (step.type === 'multiselect-language') {
      console.log(colorize(`\n${step.question}`, 'yellow'));
      const availableLangs = Object.keys(config.languages);
      const otherLangs = availableLangs.filter(l => l !== mainLang);
      
      if (otherLangs.length === 0) continue;
    ...
    else if (step.type === 'select-license') {
      console.log(colorize(`\n${step.question}`, 'yellow'));
      licenses.forEach((l, i) => console.log(`${i + 1}. ${l.name}`));
      const choice = await askQuestion(rl, `Choice (1-${licenses.length}): `);
      selectedLicense = licenses[parseInt(choice) - 1] || licenses[licenses.length - 1];
    }
    else if (step.type === 'multiselect-template') {
      console.log(colorize(`\n${step.question}`, 'yellow'));
      selectedTemplates = await promptTemplateSelection(rl, templates);
    }
  }
...
/**
 * Add Specific Mode - Drill down approach
 */
export async function runAddSpecificMode(rl) {
