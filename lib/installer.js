...
import { processFile, copyDir, colorize, sanitizeMarkdown } from './utils.js';

export async function performInstallation(mainLang, additionalLangs, selectedLicense, selectedExtras, skipReadme = false, metadata = null, selectedStyle = null) {
  ...
  if (!skipReadme) {
    await installReadme(mainLang, additionalLangs, selectedExtras, metadata, selectedStyle);
  }
  ...
  if (selectedLicense && selectedLicense.file) {
    ...
    processFile(src, dest, (content) => {
        if (metadata) {
            ...
            content = content.replace(/\[AUTHOR_NAME\]/g, metadata.AUTHOR_NAME);
        }
        return content;
    }, { metadata });
  }
  ...
  for (const extra of selectedExtras) {
    ...
    if (extra.type === 'folder') {
        if (mainLang !== 'ignore') {
            ...
            if (existsSync(src)) {
                copyDir(src, join(process.cwd(), extra.dest), { metadata });
            } else if (existsSync(fallbackSrc)) {
                 copyDir(fallbackSrc, join(process.cwd(), extra.dest), { metadata });
            }
        }
        continue;
    }
    ...
    for (const lang of allLangs) {
      // Use selectedStyle file if provided and matches this extra
      let fileName = extra.file;
      if (selectedStyle) {
          const styleObj = selectedStyle.file ? selectedStyle : selectedStyle[extra.id];
          if (styleObj) fileName = styleObj.file;
      }
      
      // Check if template exists for this language
      let src = join(config.paths.templates, `${lang}-template`, fileName);
      if (!existsSync(src)) {
...
async function installReadme(mainLang, additionalLangs, selectedExtras, metadata, selectedStyle) {
  const allLangs = [mainLang, ...additionalLangs].filter(l => l !== 'ignore');
  const styleObj = selectedStyle && !selectedStyle.file ? selectedStyle['readme'] : selectedStyle;
  const readmeFile = styleObj && styleObj.templateId === 'readme' ? styleObj.file : 'README-template.md';

  for (const lang of allLangs) {
    let src = join(config.paths.templates, `${lang}-template`, readmeFile);
    if (!existsSync(src)) {
    ...
    if (selectedExtras.length > 0) {
        const langDocs = config.docTitles?.[lang] || config.docTitles?.en || { header: '## 📂 Documentation' };
        const header = langDocs.header || '## 📂 Documentation';
        let links = `${header}\n\n`;
        
        selectedExtras.forEach(e => {
            if (e.type === 'folder') return;
            
            let name = langDocs[e.name] || e.name;
            let linkPath = `./${e.dest}`;
            links += `- [${name}](${linkPath})\n`;
        });
    ...
    content = sanitizeMarkdown(content, metadata);
